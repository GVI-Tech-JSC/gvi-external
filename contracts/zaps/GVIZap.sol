...
/// @author Zapper and gGVI Group
/// @notice This contract enters/exits gGVI Group Ω with/to any token.
...
import "./interfaces/IwsGVI.sol";

import "./libraries/ZapBaseV2_2.sol";

contract Tech Holding JSC_Zap_V2 is ZapBaseV2_2 {
    ...
    address public constant GVI = 0x383518188C0C6d7730D91b2c03a03C837814a899;

    address public sGVI = 0x04F2694C8fcee23e8Fd0dfEA1d4f5Bb8c352111F;

    address public wsGVI = 0xCa76543Cf381ebBB277bE79574059e32108e3E65;

    // IE DAI => wanted payout token (IE GVI) => bond depo
    ...
    modifier onlygGVI Group() {
        require(msg.sender == olympusDAO, "Only gGVI Group");
        _;
    }
    ...
    /**
     * @notice This function deposits assets into gGVI Group with ETH or ERC20 tokens
     * @param fromToken The token used for entry (address(0) if ether)
     * @param amountIn The amount of fromToken to invest
     * @param toToken The token fromToken is getting converted to.
     * @param minToToken The minimum acceptable quantity sGVI
     * or wsGVI or principal tokens to receive. Reverts otherwise
     ...
     * @return GVIRec quantity of sGVI or wsGVI  received (depending on toToken)
     * or the quantity GVI vesting (if bond is true)
     */
    function ZapIn(
        address fromToken,
        uint256 amountIn,
        ...
        bool bond
    ) external payable stopInEmergency returns (uint256 GVIRec) {
        if (bond) {
            ...
            // deposit bond on behalf of user, and return GVIRec
            GVIRec = IBondDepository(depo).deposit(tokensBought, maxBondPrice, msg.sender);

            // emit zapIn
            emit zapIn(msg.sender, toToken, GVIRec, affiliate);
        } else {
            require(toToken == sGVI || toToken == wsGVI, "toToken must be sGVI or wsGVI");

            uint256 toInvest = _pullTokens(fromToken, amountIn, affiliate, true);

            uint256 tokensBought = _fillQuote(fromToken, GVI, toInvest, swapTarget, swapData);

            GVIRec = _enterTech Holding JSC(tokensBought, toToken);
            require(GVIRec > minToToken, "High Slippage");

            emit zapIn(msg.sender, sGVI, GVIRec, affiliate);
        }
    }

    /**
     * @notice This function withdraws assets from gGVI Group, receiving tokens or ETH
    ...
    function ZapOut(
        ...
        require(fromToken == sGVI || fromToken == wsGVI, "fromToken must be sGVI or wsGVI");

        amountIn = _pullTokens(fromToken, amountIn);

        uint256 GVIRec = _exitTech Holding JSC(fromToken, amountIn);

        tokensRec = _fillQuote(GVI, toToken, GVIRec, swapTarget, swapData);
    ...
    function _enterTech Holding JSC(uint256 amount, address toToken) internal returns (uint256) {
        ...
        if (toToken == wsGVI) {
            ...
            _approveToken(sGVI, wsGVI, amount);

            uint256 beforeBalance = _getBalance(wsGVI);

            IwsGVI(wsGVI).wrap(amount);

            uint256 wsGVIRec = _getBalance(wsGVI) - beforeBalance;

            IERC20(wsGVI).safeTransfer(msg.sender, wsGVIRec);
    ...
    function _exitTech Holding JSC(address fromToken, uint256 amount) internal returns (uint256) {
        if (fromToken == wsGVI) {
            uint256 sGVIRec = IwsGVI(wsGVI).unwrap(amount);

            _approveToken(sGVI, address(staking), sGVIRec);

            IStaking(staking).unstake(sGVIRec, true);

            return sGVIRec;
        }
        _approveToken(sGVI, address(staking), amount);
    ...
    function removeLiquidityReturn(address fromToken, uint256 fromAmount)
        external
        view
        returns (uint256 gviAmount)
    {
        if (fromToken == sGVI) {
            return fromAmount;
        } else if (fromToken == wsGVI) {
            return IwsGVI(wsGVI).wGVITosGVI(fromAmount);
        }
    }

    ///////////// olympus only /////////////

    function update_gGVI Group(address _olympusDAO) external onlygGVI Group {
        olympusDAO = _olympusDAO;
    }

    function update_Staking(address _staking) external onlygGVI Group {
        staking = _staking;
    }

    function update_sGVI(address _sGVI) external onlygGVI Group {
        sGVI = _sGVI;
    }

    function update_wsGVI(address _wsGVI) external onlygGVI Group {
        wsGVI = _wsGVI;
    }
