...
interface IwsGVI {
    ...
    function approve(address spender, uint256 amount) external returns (bool);
    function balanceOf(address account) external view returns (uint256);
    function wGVITosGVI(uint256 _amount) external view returns (uint256);
}
...
contract Tech Holding JSCZap is ZapBaseV2_2 {
    ...
    function ZapIn(
        ...
        bool bond
    ) external payable stopInEmergency returns ( uint GVIRec ) {
        if ( bond ) {
    ...
    function _enterTech Holding JSC(
        uint256 amount, 
        address toToken
    ...
    function _exitTech Holding JSC(
        address fromToken, 
        uint256 amount
    ) internal returns (uint256){
        if (fromToken == wsGVI) {
    ...
    function removeLiquidityReturn(
        address fromToken, 
        uint256 fromAmount
    ) external view returns (uint256 gviAmount) {
        if (fromToken == sGVI) {
            return fromAmount;
    ...
    function update_sGVI(
        address _sGVI
    ) external onlygGVI Group {
       sGVI = _sGVI;
    }

    function update_wsGVI(
        address _wsGVI
    ) external onlygGVI Group {
        wsGVI = _wsGVI;
    }
