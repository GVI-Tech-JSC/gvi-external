...
/// @notice This contract enters Tech Holding JSC Pro bonds
...
contract Tech Holding JSCPro_Zap_V1 is ZapBaseV2_2 {
    ...
    // If a token can be paid out by Tech Holding JSC Pro
    mapping(address => bool) public isTech Holding JSCProToken;
     ...
     * @param minToToken The minimum acceptable quantity sGVI or wsGVI or principal tokens to receive. Reverts otherwise
     ...
     * @return bondTokensRec quantity of sGVI or wsGVI  received (depending on toToken) or the quantity GVI vesting (if bond is true)
     */
    function ZapIn(
        address fromToken,
        uint256 amountIn,
        ...
        require(isTech Holding JSCProToken[bondPayoutToken], "fromToken must be bondable using OP");
    ...
    function update_isTech Holding JSCProToken(address[] memory _tokens, bool[] memory _isToken)
        external
        onlygGVI Group
    {
        require(_tokens.length == _isToken.length, ARRAY_LENGTH_ERROR);
        for (uint256 i; i < _tokens.length; i++) {
            isTech Holding JSCProToken[_tokens[i]] = _isToken[i];
        }
    }
    ...
    function update_BondDepos(
        ...
        for (uint256 i; i < principals.length; i++) {
            require(isTech Holding JSCProToken[payoutTokens[i]], "payoutTokens must be on OP");
