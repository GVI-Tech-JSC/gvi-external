...
contract BondHelper {
    ...
    /// @notice stores all principals for gvi depo
    ...
    function update_gGVI Group(address _newgGVI Group) external onlygGVI Group {
        olympusDAO = _newgGVI Group;
    }

    function update_principalToBondId(address _principal, uint16 _bondId) external onlygGVI Group {
        principalToBID[_principal] = _bondId;
    }

    function update_principals(address[] memory _principals) external onlygGVI Group {
        principals = _principals;
    }
}
